...
int set_data2axis(void) {
  ...
  do {
    if (*tokenptr == 'D') {
      data2axis[field++] |= (X_DOWN | (xd << 8)); /* we don't use the MSB it actually */
      xd <<= 1;
    } else if (*tokenptr == 'T') { /* we don't use the MSB it actually */
      data2axis[field++] |= X_TOP;
      xt << 1;
    } else if (*tokenptr == 'L') {
